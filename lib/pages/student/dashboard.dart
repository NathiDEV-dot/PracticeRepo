...
class _StudentDashboardState extends State<StudentDashboard> {
  ...
  void _calculateProgress() {
    int completedCount =
        _lessonCompletion.values.where((completed) => completed).length;
    int homeworkCount =
        _homeworkCompletion.values.where((completed) => completed).length;
  ...
  Widget _buildHomeworkList(List<Map<String, dynamic>> homeworkList) {
    return Column(
      children:
          homeworkList.map((homework) => _buildHomeworkCard(homework)).toList(),
    );
  }
  ...
  Widget _buildActivityBreakdown() {
    ...
    return Column(
      ...
      children: [
        ...
        _buildBreakdownItem(
            'Lessons Watched', completedLessons, Icons.video_library),
        const SizedBox(height: 8),
        _buildBreakdownItem('Homework Completed', completedHomework,
      ...
      child: Column(
        children: [
          _buildProfileStatItem(
              'Activities Completed', '$completed', Icons.check_circle),
          const SizedBox(height: 16),
          _buildProfileStatItem(
              'Total Activities', '$total', Icons.video_library),
          const SizedBox(height: 16),
